...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save') {
    ...
    if ($name === '' || !$department_id) {
        $error = 'Department and Model Name are required.';
    } else {
        if ($id !== '') {
            $stmt = $pdo->prepare('UPDATE m_assy_model SET department_id = ?, name = ?, sort_order = ? WHERE id = ?');
        ...
        header('Location: assy_models.php?department_id=' . $department_id . '&saved=1');
        exit;
    }
}
...
if (($_GET['action'] ?? '') === 'toggle' && isset($_GET['id'])) {
    ...
    header('Location: assy_models.php?department_id=' . (int)($_GET['department_id'] ?? 0));
    exit;
}
...
if (($_GET['action'] ?? '') === 'delete' && isset($_GET['id'])) {
    try {
        ...
        $stmt->execute([(int)$_GET['id']]);
        header('Location: assy_models.php?department_id=' . (int)($_GET['department_id'] ?? 0) . '&deleted=1');
        exit;
    } catch (PDOException $e) {
        $error = 'Cannot delete, this model is already used by a checking item / checksheet. Deactivate it instead.';
    }
}
...
<form method="post" class="admin-form">
    ...
    <div class="form-row">
        ...
        <?php if ($editRow): ?><a href="assy_models.php?department_id=<?= $selected_department_id ?>" class="btn btn-secondary">Cancel</a><?php endif; ?>
    </div>
</form>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="row-actions">
                <a href="assy_models.php?department_id=<?= $selected_department_id ?>&action=edit&id=<?= $row['id'] ?>">Edit</a>
                <a href="assy_models.php?department_id=<?= $selected_department_id ?>&action=toggle&id=<?= $row['id'] ?>"><?= $row['is_active'] ? 'Deactivate' : 'Activate' ?></a>
                <a href="assy_models.php?department_id=<?= $selected_department_id ?>&action=delete&id=<?= $row['id'] ?>" onclick="return confirm('Delete this model?')" class="danger">Delete</a>
            </td>
        </tr>
        <?php endforeach; ?>
        <?php if (!$rows): ?><tr><td colspan="4" class="empty">No models for this department yet.</td></tr><?php endif; ?>
    </tbody>
</table>
</div>
