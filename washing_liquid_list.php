...
// Checked By entries scoped strictly to this section — keeps Washing's
// Foreman/Supervisor lists from mixing with Torque/Sub Assembly/FO Pump
// Assy's, which share the same department_id. Routing lives entirely in
// admin/users.php now; unassigned m_checker rows are stale leftovers, not
// a fallback to show here.
$stmt = $pdo->prepare(
    "SELECT c.* FROM m_checker c
     WHERE c.department_id = ? AND c.is_active = 1 AND c.role = 'foreman'
       AND c.section_id = (SELECT id FROM m_checksheet_section WHERE department_id = ? AND route = 'washing_liquid_list.php')
     ORDER BY c.name"
);
