...
// Checked By entries scoped strictly to this section — keeps Torque's
// Checked By list from mixing with Washing/Sub Assembly/FO Pump Assy's,
// which share the same department_id. Routing lives entirely in
...
$stmt = $pdo->prepare(
    "SELECT c.* FROM m_checker c
     WHERE c.department_id = ? AND c.is_active = 1
       AND c.section_id = (SELECT id FROM m_checksheet_section WHERE department_id = ? AND route = 'assembly_list.php')
     ORDER BY c.name"
);
