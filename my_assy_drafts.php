...
if (($_GET['action'] ?? '') === 'delete' && isset($_GET['id'])) {
    $stmt = $pdo->prepare("DELETE FROM t_assy_header WHERE id = ? AND status = 'draft'");
    $stmt->execute([(int)$_GET['id']]);
    header('Location: my_assy_drafts.php?deleted=1');
    exit;
}
...
$page_subtitle = 'Assembly checksheets not yet submitted';
...
<div class="cs-card-list">
    ...
    <div class="cs-card">
        ...
        <div class="cs-card-actions">
            <a href="assembly_list.php?department_id=<?= $row['department_id'] ?>&draft_id=<?= $row['id'] ?>" class="cs-view-btn">Continue</a>
            <a href="my_assy_drafts.php?action=delete&id=<?= $row['id'] ?>" onclick="return confirm('Delete this draft?')" class="cs-delete-link">Delete</a>
        </div>
    </div>
    <?php endforeach; ?>
